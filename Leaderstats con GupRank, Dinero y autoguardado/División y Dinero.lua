...
local id = 7 ---- Aqui colocar el id del grupo de roblox
...
game.Players.PlayerAdded:Connect(function(player) ---. Al momento de un jugador conectarse.
	...
	local GroupRank = Instance.new("StringValue", leader)
	GroupRank.Name = "Rango" ---- nombre del rank o rango que aparecera en el leaderstats
	GroupRank.Value = player:GetRoleInGroup(id) ---- obtencion del rango 
	local data
...
game.Players.PlayerRemoving:Connect(function(player)
	local success, err = pcall(function()
		Dinero_value:SetAsync(player.UserId, player.leaderstats:WaitForChild('Pesos').Value) ----Modificar "WaitForChild('Nombre de tu varible')
	end)
